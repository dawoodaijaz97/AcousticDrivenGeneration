...
4. ~~**B23:** train structured seven-label targets~~ — **done; closed** (label match **0.0**, verbalized AVG **0.315** vs B17 **0.542**; see [training_progress.md](training_progress.md)).
5. **Supervisor contact (2026-06-28):** emailed supervisor with progress summary, Flan-T5 plateau (~**0.542** AVG vs paper ~**0.68**), and request for his code. **Code received** (`Speech_LLM/`); key difference vs our pipeline is **raw CSV `Instructions`** as encoder input (not compact seven-score `input_text`).
6. ~~**B24 — raw `Instructions` input**~~ — **done; closed (2026-07-13)** (`--input-format instructions-raw`, B11 recipe: flan-paper, 5 ep, 5e-4). AVG **0.479** vs **B11 0.538** / **B17 0.542** — raw input **regresses** on every decode metric despite best-ever `test_loss` **1.026**; input compression is not the plateau cause (see [training_progress.md](training_progress.md)).
...
- [x] **B23** — train **structured seven-label target** (`Breathing: Mild`, …); **`main.prepare --target-format structured-seven-label`**, train, **`main.eval_structured`** — **closed:** label exact match **0.0**, verbalized AVG **0.315** (vs B17 **0.542**)
- [x] **B24** — **raw CSV `Instructions`** encoder input (`--input-format instructions-raw`); B11 recipe (flan-paper, 5 ep, 5e-4, `max_source_length` **512**) — AVG **0.479** (vs B11 **0.538**, B17 **0.542**); PD **0.468** / HC **0.491**; **raw-input lever closed** (Phase 6)
...
- [x] Use structure metrics as promotion gates: do not promote a run that improves AVG while keeping category coverage near **0.143** — **applied:** B21/B22/B23 all failed gate vs B17
...
- [x] **B24** — raw CSV **`Instructions`** as encoder input (`--input-format instructions-raw`) — AVG **0.479** (below B11 **0.538**); **closed** (Phase 6)
...
### Phase 5 — Structure-forcing and schema reliability — closed (2026-06-24)
...
- [x] **D1 — synthetic-vs-real report distribution audit** — **complete (2026-06-24)**
  ...
  - **Result:** train/val/test targets all **7-slot coverage 1.0**; synthetic duplicate rate **~95%** train; model decode **~14%** — led to **B23** (also **closed**, label match **0.0**)

- [x] **B23 — structured target training** — **closed (2026-06-24)**
  - Data: `data/processed/flan-t5-base/100k-structured-seven-label/`; LoRA r=32 on B11 `final_model`, 3 ep
  - **Result:** label exact match **0.0**, decode label coverage **0.0**; verbalized AVG **0.315** vs B17 **0.542** (−**0.227**); `test_loss` **3.375** vs B17 **1.145** — model did not learn seven-label prediction; verbalized structure **1.0** is padding artifact
  - Artifacts: `runs/flan-t5-base/100k-structured-seven-label/`

**Phase 5 verdict:** post-process (B21), rerank (B22), audit (D1), and structured targets (B23) all **closed** without beating **B17**. Resume broad training sweeps only under a **new hypothesis** (not more structure-forcing on this recipe).

### Phase 6 — Supervisor baseline alignment (Speech_LLM) — closed (2026-07-13)

**Goal:** test whether the professor’s **raw `Instructions`** field (full multi-biomarker string) improves decode vs our ETL **compact seven-score** `input_text`, holding training recipe fixed.

**Hypothesis:** B11/B17 plateau may partly reflect **input compression** (7 parsed scores) rather than model capacity; `Speech_LLM/` feeds `Instructions` unchanged.

- [x] **`main.input_formats` + `--input-format`** in `main.prepare` (`compact-seven` default; `instructions-raw` → `instructions_raw` column)
- [x] HPC scripts: `scripts/hpc/prepare_flan_t5_base_b24_raw_instructions.sh`, `train_flan_t5_base_b24_raw_instructions_5ep_a100.slurm`, `eval_flan_t5_base_b24_5ep_a100.slurm`
- [x] **B24 prepare** → `data/processed/flan-t5-base/100k-flan-paper-raw-instructions/tokenized/`
- [x] **B24 train** → `runs/flan-t5-base/100k-flan-paper-raw-instructions-5ep/` (B11 hyperparams: 5 ep, 5e-4, wd 0.01); `test_loss` **1.026**
- [x] **B24 `eval_decode`** → logged in [training_progress.md](training_progress.md); AVG **0.479** < B17 **0.542** — **promotion gate failed, not promoted**
- [ ] Optional follow-ups (only if B24 wins) — **not pursued** (B24 lost): professor decode settings (`do_sample`, `min_length=100`); LoRA on B24 checkpoint; `max_source_length` 128 vs 512 ablation

**Phase 6 verdict (2026-07-13):** raw `Instructions` input **regresses** — AVG **0.479** vs B11 **0.538** / B17 **0.542**; PD **0.468** / HC **0.491**; regression on every decode metric despite the lowest `test_loss` yet (**1.026**, not comparable across input formats). The **input-compression hypothesis is rejected**: the ETL compact seven-score `input_text` outperforms raw `Instructions` under an identical recipe. **B17 remains the reporting config.** Any Speech_LLM advantage must come from model family/scale or decode settings, not input format.

**B24 config (fixed for first run):**

| Setting | B24 | B11 (comparison) |
|---------|-----|------------------|
| Input | `instructions-raw` | `compact-seven` (`input_text`) |
| Prompt | `flan-paper` | `flan-paper` |
| `max_source_length` | **512** | **256** |
| Train | 5 ep, 5e-4, wd 0.01 | same |
...
| Category coverage / slot coverage metrics | [x] (B21/B22/B23 logged in eval JSON) |
| Structure-aware promotion gate | [x] (applied: B21/B22/B23 all failed vs B17) |
...
| Structured seven-label target format (B23) | [x] (closed: label match 0.0, verbalized AVG 0.315) |
| Raw CSV `Instructions` input (B24) | [x] (closed: AVG 0.479, below B11 0.538 / B17 0.542) |
| `max_source_length` / `max_target_length` audit | [x] (B5 compact data; 256/512 OK — B24 re-audit moot, raw-input lever closed) |
...
| Encoder input format (`compact-seven` vs `instructions-raw`) | [x] **B24** closed — compact-seven wins (0.538 vs 0.479) |
...
| **D1** | data audit | train/val/test | n/a | n/a | [x] | `analysis/synthetic_real_report_audit.json` | **Done:** target coverage **1.0**; high synthetic duplicates; B23 follow-up also **closed**
| **B23** | flan-t5-base | 100k | structured-seven-label | 5e-4 | [x] | `runs/flan-t5-base/100k-structured-seven-label` | LoRA r=32 on B11, 3 ep; label exact match **0.0**; verbalized AVG **0.315** (vs B17 **0.542**); **structured-target closed** |
| **B24** | flan-t5-base | 100k | flan-paper | 5e-4 | [x] | `runs/flan-t5-base/100k-flan-paper-raw-instructions-5ep` | **`--input-format instructions-raw`**; B11 recipe; AVG **0.479** (vs B11 **0.538** / B17 **0.542**); PD **0.468** / HC **0.491**; `test_loss` **1.026**; **raw-input closed** (Phase 6) |
...
24. [x] **B23 — structured seven-label target training** — **closed:** label exact match **0.0**, verbalized AVG **0.315** (vs B17 **0.542**); Phase 5 structure levers exhausted. Logged in [training_progress.md](training_progress.md).
...
26. [x] **Contact supervisor (2026-06-28)** — emailed progress update, plateau on Flan-T5 family, and request for his code. **Code received** (`Speech_LLM/`).
27. [x] **B24 pipeline** — `main/input_formats.py`, `main.prepare --input-format instructions-raw`, HPC prepare/train/eval scripts.
28. [x] **B24 prepare + train + eval_decode** — **closed (2026-07-13):** AVG **0.479** (vs B11 **0.538** / B17 **0.542**); raw `Instructions` input regresses; **B17 remains reporting config**. Logged in [training_progress.md](training_progress.md).

---

## Commands cheat sheet

**B24 prepare (CPU — raw Instructions, re-tokenize required):**
...
sed -i 's/\r$//' scripts/hpc/prepare_flan_t5_base_b24_raw_instructions.sh
bash scripts/hpc/prepare_flan_t5_base_b24_raw_instructions.sh
```

**B24 train (A100 — B11 recipe on raw-instructions tokenized data):**

```bash
sed -i 's/\r$//' scripts/hpc/train_flan_t5_base_b24_raw_instructions_5ep_a100.slurm
sbatch.tinygpu scripts/hpc/train_flan_t5_base_b24_raw_instructions_5ep_a100.slurm
```

**B24 decode eval (GPU — after train):**
...
python -m main.eval_decode \
  --tokenized-dir data/processed/flan-t5-base/100k-flan-paper-raw-instructions/tokenized \
  --model-path runs/flan-t5-base/100k-flan-paper-raw-instructions-5ep/final_model \
  --tokenizer-model $WORK/models/flan-t5-base \
  --output-json runs/flan-t5-base/100k-flan-paper-raw-instructions-5ep/test_decode_metrics.json \
  --batch-size 8 --seed 42 --require-gpu
```

Or batch eval:

```bash
sed -i 's/\r$//' scripts/hpc/eval_flan_t5_base_b24_5ep_a100.slurm
sbatch.tinygpu scripts/hpc/eval_flan_t5_base_b24_5ep_a100.slurm
```
...
- More broad LR/epoch/model-size sweeps **without a new hypothesis** — Phase 5 (B21/B22/D1/B23) and Phase 6 (**B24** raw input, AVG **0.479**) **closed**; **B17 remains best**
...
*Last updated: 2026-07-13. **B17** remains best reporting config (AVG **0.542**). **Phase 6 closed** — **B24** raw `Instructions` input AVG **0.479** (regression on every decode metric); input format is not the plateau cause. Plan only — mark `[x]` when done; record numbers in [training_progress.md](training_progress.md).*
