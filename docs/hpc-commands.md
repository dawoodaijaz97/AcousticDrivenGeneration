...
**Wall time:** TinyGPU `#SBATCH --time=` is capped at **`24:00:00`** per job. For **base** model training (`flan-t5-base`/`t5-base`), request **`10:00:00`** — that's enough for LoRA/5-ep/oversampled base runs and clears the queue faster than reserving the full 24 h. Reserve `24:00:00` for **large** model training; eval jobs typically use `02:00:00`. If a run may exceed the requested time, split across checkpoints (`--resume-from-checkpoint auto`) or reduce epochs — do not request >24 h.
...
Examples: `scripts/hpc/train_flan_t5_base_100k_flan_paper_a100.slurm` (B5), `train_flan_t5_small_100k_lr5e4_a100.slurm` (S4), `train_flan_t5_base_b20_pd_oversample_lora32_a100.slurm` (B20).
