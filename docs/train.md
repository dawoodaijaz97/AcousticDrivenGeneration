...
For T5-style models, [`main/prompts.py`](../main/prompts.py) prepends a task prefix to build **`source_text`** (see `--prompt-style` in `main.prepare`: **`default`** = mFDA biomarker instruction; **`flan-paper`** = `Generate a report for:`). [`main/tokenization.py`](../main/tokenization.py) tokenizes that into **`input_ids`** / **`attention_mask`** for the **encoder**.
