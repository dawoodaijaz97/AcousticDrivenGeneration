...
| B23 | flan-t5-base | 100k | 5e-4 | 3.375 | 0.267 | 0.080 | 0.266 | 0.219 | 0.744 | **0.315** |
| B24 | flan-t5-base | 100k | 5e-4 | 1.026 | 0.530 | 0.299 | 0.443 | 0.320 | 0.805 | **0.479** |

↓ lower is better for `test_loss`; ↑ higher is better for decode metrics. **B17** = B11 checkpoint + **LoRA rank 32**, 3 ep — **best reporting config** (AVG **0.542**). **B23** = seven-label targets + LoRA r=32 on B11, 3 ep; **`main.eval_structured`** verbalized AVG **0.315** (−**0.227** vs B17), label exact match **0.0**, decode label coverage **0.0** — **structured-target lever closed**. **B22** = B17 + structure rerank (beams **8**, **5** candidates); AVG **0.531** (−**0.011** vs B17), **`category_coverage` 0.144** unchanged — **rerank lever closed**. **B21** = forced seven-slot post-process; structured AVG **0.492** — **post-process closed**. **B20** ≈ tie B17; **B19** template prompt **closed**; **B18** longer LoRA **closed**. **B24** = raw CSV `Instructions` encoder input on B11 recipe; AVG **0.479** (−**0.059** vs B11, −**0.063** vs B17) despite best-ever `test_loss` **1.026** — **raw-input lever closed**. *B23 AVG = verbalized prose after deterministic template (not raw decode).*

**Run directories:** `runs/flan-t5-small/{100k,100k-lr5e4,100k-lr3e4,10k-lr1e4,10k-lr3e4,10k-lr5e4,1k}`, `runs/flan-t5-base/{100k,100k-lr5e4,100k-flan-paper,100k-flan-paper-categories,100k-flan-paper-numeric-labels,100k-flan-paper-ls002,100k-flan-paper-ls005,100k-flan-paper-ls010,100k-flan-paper-5ep,100k-flan-paper-5ep-wd0,100k-flan-paper-5ep-wd005,100k-flan-paper-5ep-lora8,100k-flan-paper-5ep-lora16,100k-flan-paper-5ep-lora32,100k-flan-paper-5ep-lora32-5ep,100k-flan-paper-5ep-lora32-structured-decode,100k-flan-paper-5ep-lora32-rerank,100k-flan-paper-report-template-lora32,100k-flan-paper-5ep-freeze-enc,100k-structured-seven-label,100k-flan-paper-raw-instructions-5ep}`, `runs/flan-t5-large/{100k,100k-lr5e4,100k-flan-paper-5ep}`, `runs/t5-base/100k-flan-paper-5ep`.
...
| B23 | 0.242 | 0.388 | 0.302 |
| B24 | 0.468 | 0.491 | 0.338 |

*B21 PD/HC = structured (forced template) decode metrics; raw decode ≈ B17 (PD **0.513**, HC **0.571**). B22 = structure rerank on B17 (beams 8, 5 candidates). B23 PD/HC = verbalized prose AVG from `main.eval_structured` (label decode coverage **0.0**).*
...
- **Takeaway:** **Post-process template lever closed** — format forcing without model-learned content hurts real-test metrics. **Keep B17** for reporting.
...
- **Takeaway:** **Structure reranking lever closed** — multi-candidate decode does not surface fuller 7-slot hypotheses. **Keep B17** for reporting.
...
- **Takeaway:** Model decode **~14%** slot coverage vs **100%** in training targets → failure is **generation/training dynamics**, not absent structure in data. **Decode-only fixes (B21/B22) confirmed insufficient.**

### Phase 5 — structured seven-label target training (B23) — closed (2026-06-24)

- **B23** (LoRA r=32 on B11 `final_model`, 3 ep, **`--target-format structured-seven-label`**, LR **5e-4**) trains on compact `Category: Severity` targets instead of full prose.
- **`test_loss` 3.375** vs B17 **1.145** — model did not learn the label task under this recipe (initialized from prose B11, not fresh).
- **Label decode:** exact match **0.0**, per-category exact match **0.0** all slots; **`label_structure_metrics` coverage 0.0** / **`all_7_slots_rate` 0.0** — raw model output is not parseable as seven labels.
- **Verbalized prose** (deterministic template on predicted labels): AVG **0.315** vs B17 **0.542** (−**0.227**); R-1 **0.267**, R-2 **0.080**, BLEU **0.219**, BERT **0.744**.
- **Verbalized structure metrics** show coverage **1.0** — **misleading** (verbalizer pads missing labels with defaults, same artifact as B21).
- **By group (verbalized):** **PD AVG 0.242** vs B17 **0.513**; **HC AVG 0.388** vs B17 **0.571**; HC BLEU **0.302** vs **0.383**.
- **Promotion gate failed:** no label accuracy and large prose regression vs B17.
- **Takeaway:** **Structured-target lever closed** for this pipeline — simpler targets + LoRA-from-prose did not teach category/severity prediction. **Keep B17** for reporting. **Phase 5 structure levers exhausted** (B21 post-process, B22 rerank, D1 audit, B23 targets).
- **Artifacts:** `runs/flan-t5-base/100k-structured-seven-label/test_decode_metrics.json`, `test_eval.json`.

### Phase 6 — raw `Instructions` encoder input (B24) — closed (2026-07-13)

- **B24** (full fine-tune from `flan-t5-base`, B11 recipe: flan-paper, **5 ep**, LR **5e-4**, wd **0.01**; **`--input-format instructions-raw`**, `max_source_length` **512**) feeds the professor's raw CSV `Instructions` string to the encoder instead of the ETL compact seven-score `input_text`.
- **Decode regresses on every metric:** AVG **0.479** vs B11 **0.538** (−**0.059**) and B17 **0.542** (−**0.063**); R-1 **0.530** vs **0.600**, R-2 **0.299** vs **0.384**, R-L **0.443** vs **0.527**, BLEU **0.320** vs **0.358**, BERT **0.805** vs **0.822** (all vs B11).
- **`test_loss` 1.026** — the lowest of any 100k base run (B11 **1.094**, B17 **1.145**) — but it is computed on *different encoder inputs* (same targets), so it is not directly comparable across input formats; either way decode quality is clearly worse. Yet another loss-vs-generation divergence.
- **Generation is healthy, not degenerate:** sacrebleu precisions **64.6 / 45.5 / 33.0 / 24.5**, BP **0.815** — coherent text, just a worse match to references than the compact-input models.
- **By group:** **PD AVG 0.468** vs B11 **0.510** (−0.042); **HC AVG 0.491** vs **0.567** (−0.076); HC BLEU **0.338** vs **0.379**. Both groups regress, concentrated on HC (same pattern as B12/N0 regressions).
- **Takeaway:** the **input-compression hypothesis is not supported** — parsing the seven scores into compact `input_text` *helps* rather than hurts at base scale. **Raw-input lever closed; keep B17 for reporting.** Whatever advantage `Speech_LLM/` has must come from something other than raw `Instructions` input (model family/scale, decode settings). Phase 6 optional follow-ups (professor decode settings, LoRA on B24, `max_source_length` ablation) were conditioned on a B24 win and are **dropped**.
- **Artifacts:** `runs/flan-t5-base/100k-flan-paper-raw-instructions-5ep/test_decode_metrics.json`, `test_eval.json`.
...
- **`test_loss` 1.146** vs B17 **1.145** (≈ tie). Train used ~150k rows after 2× duplicate of Moderate+ synthetic reports (PD/HC labels absent on train).
- **By group:** **PD AVG 0.513** vs B17 **0.513** (tie); **HC AVG 0.570** vs **0.571** (−0.001); HC BLEU **0.384** vs **0.383**.
- **PD structure (`pd_analysis.json`):** `mean_coverage_hyp` **0.143** (≈ unchanged); Breathing **100%** hyp presence; Lips–Intelligibility still **0–1%**; PD severity match **0.479** (unchanged).
- **Takeaway:** **Severity oversampling lever closed** — duplicating Moderate+ training rows does not lift real-test PD decode or 7-slot structure. **Keep B17** as reporting config.
...
- **Best run to date:** **B17 — LoRA rank 32 on B11 checkpoint, 3 ep** (**AVG 0.542**). **Phase 5 closed:** B21 post-process (AVG **0.492**), B22 rerank (AVG **0.531**, coverage **0.144**), B23 structured targets (verbalized AVG **0.315**, label match **0.0**). **Phase 6 closed:** B24 raw `Instructions` input (AVG **0.479**).
...
See **[Model Improvement Plan](Model%20Improvement%20Plan.md)**. Best reporting config remains **B17** (AVG **0.542**). **Phase 5 complete** — post-process, rerank, audit, and structured-target training all **closed** without beating B17. **Phase 6 complete** — B24 raw `Instructions` input **closed** (AVG **0.479**); input format is not the gap vs the paper. Further gains likely need a **new direction** (e.g. train labels from scratch without prose init, larger model family, professor's decode settings on B17, or accept B17 as final for this track).
